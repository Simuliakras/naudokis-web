...
export async function generateMetadata({ params }: LayoutProps<"/[lang]">): Promise<Metadata> {
  ...
  return {
    ...
    // Search-engine ownership proof. Both are optional: with no token configured
    // nothing is emitted, so unconfigured builds are byte-identical. Set the
    // Google one to verify the property in Search Console without needing DNS
    // access at launch. Baked at BUILD time — see .env.example.
    ...
    icons: {
      icon: [
        // Declared explicitly rather than relying on the app/favicon.ico file
        // convention: `icons` metadata is authored here, so the <link> set stays
        // in one place. Generated by `yarn gen:favicon` — legacy crawlers and
        // link-preview services still request /favicon.ico by convention.
        // `sizes` is hand-synced with SIZES in scripts/generate-favicon.mjs.
