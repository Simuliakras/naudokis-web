...
export function Nav({ onSearch }: { onSearch: () => void }) {
  const { dict } = useI18n();
  return (
    <header className="nk-nav-bar" style={{ position: "sticky", top: 0, zIndex: 50, background: "var(--nk-glass-nav)", backdropFilter: "blur(20px)", borderBottom: "1px solid color-mix(in srgb, var(--nk-border) 40%, transparent)" }}>
      <div className="nk-container" style={{ height: 92, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 32 }}>
          ...
          <a className="nk-nav nk-link" href="#kategorijos">{dict.nav.category}</a>
...
function LocaleSwitcher() {
  ...
  const href = (l: Locale) => (l === defaultLocale ? "/" : `/${l}`);
  return (
...
function SearchBar({ query, setQuery }: { query: string; setQuery: (v: string) => void }) {
  const { dict } = useI18n();
  return (
    <form className="nk-search" onSubmit={(e) => { e.preventDefault(); const el = document.getElementById("skelbimai"); if (el) el.scrollIntoView({ behavior: "smooth" }); }} style={{
      ...
      <span style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={dict.search.placeholder}
      ...
      <span style={{ display: "flex", alignItems: "center", gap: 12, padding: "0 8px" }}>
        <Icon name="MapPin" size={20} color="var(--nk-bg)" stroke={2} />
        <span style={{ fontFamily: "var(--nk-font-body)", fontSize: 18, color: "var(--nk-text-slate)" }}>{dict.search.where}</span>
      </span>
...
export function Categories({
  ...
  const all = data ?? [];
  const list = all.filter((c) => c.title.toLowerCase().includes(q)).slice(0, 6);
  return (
    <section id="kategorijos" className="nk-container" style={{ paddingBlock: "clamp(72px, 10vw, 120px)" }}>
      <SectionHead eyebrow={t.eyebrow} title={t.title}
        action={<Button variant="outline">{t.all}</Button>} />
      {isLoading ? (
        ...
        <div className="nk-grid-6 nk-reveal" style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 20 }}>
          {list.map((c) => <CategoryCard key={c.id} title={c.title} tint={c.tint} onOpen={() => onOpen({ title: c.title, city: dict.common.sampleCity, price: dict.common.samplePrice })} />)}
        </div>
      ) : (
        <EmptyState icon="LayoutGrid"
          title={t.emptyTitle}
          subtitle={t.emptySubtitle(query)}
          actionLabel={t.emptyAction} onAction={onReset} />
      )}
    </section>
  );
}
...
export function Offers({
  ...
  const { data, isLoading, isError, refetch } = useListings(locale, debouncedQuery);
      ...
      <div className="nk-container" style={{ position: "relative", paddingBlock: 80 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 24, marginTop: 64, flexWrap: "wrap" }}>
          <nav style={{ display: "flex", gap: 34 }}>
            {dict.footer.links.map((t) => (
              <a key={t} className="nk-nav nk-link" href="#" aria-disabled="true" onClick={(e) => e.preventDefault()} style={{ color: "var(--nk-text-2)" }}>{t}</a>
            ))}
          </nav>
