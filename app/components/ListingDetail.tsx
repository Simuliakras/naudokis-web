...
// Compact icon "fact card" used by the rental-terms grid. Reading order is
// sub (muted eyebrow) → title (bold value) → detail (muted follow-on) — so `sub`
// must always be a label that stands BEFORE the bold line; a clause that refers
// back to the title belongs in `detail`, never in `sub`.
function FactCard({
  icon,
  title,
  sub,
  ...
  return (
    <div
      ...
      style={{
        ...
        alignItems: "center",
        gap: "var(--nk-gap-md)",
        padding: "var(--nk-card-pad-sm)",
        borderRadius: 16,
        background: "var(--nk-surface-glass)",
        border: "1px solid var(--nk-border)",
      }}
    >
      <span
        className="nk-fact__disk"
        style={{
          width: 46,
          height: 46,
          borderRadius: 13,
          flex: "none",
          background: "var(--nk-purple-tag)",
          display: "flex",
          alignItems: "center",
    ...
    <span style={{ display: "inline-flex", gap: 2 }}>
      {Array.from({ length: 5 }).map((_, i) => (
        <Icon
          ...
          size={size}
          color="var(--nk-yellow)"
          fill={i < Math.round(value) ? "var(--nk-yellow)" : "none"}
        />
      ))}
    </span>
  );
...
function GalleryTile({
  ...
  const inner = (
    <>
      ...
      {!showPhoto && (
        <Icon
          name={src ? "ImageOff" : "Image"}
          size={big ? 64 : 34}
          stroke={1.4}
          className="nk-imgicon"
        />
      )}
  ...
  if (onOpen && showPhoto) {
    return (
      <button
        type="button"
        onClick={onOpen}
        className="nk-imgph nk-gtile nk-gtile--btn"
        style={{ borderRadius: "var(--nk-r-tile)", position: "relative" }}
      >
        {inner}
      </button>
    );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap }}>
      {Array.from({ length: rows }).map((_, i) => (
        <Skel key={i} h={13} w={i === rows - 1 ? last : "100%"} />
      ))}
    </div>
  );
}
...
export function ListingSkeleton() {
  const card: React.CSSProperties = {
    borderRadius: "var(--nk-r-card)",
    padding: "var(--nk-card-pad)",
    display: "flex",
    flexDirection: "column",
    gap: "var(--nk-gap-md)",
  };
  ...
  // BookingPanel in order and height — price header → date field → trust rows →
  // reserve CTA + its note — so neither breakpoint reflows when the content
  // lands. The date field's 58px is the real trigger's min-height (see
  // .nk-cal-trigger); the picker itself mounts client-side but the FIELD renders
  // server-side, so this space is genuinely reclaimed, not deferred.
  const bookingSkel = (withButton: boolean) => (
    ...
    >
      ...
      <Skel h={58} r={14} />
      {/* the two trust rows (deposit / cancellation) */}
      ...
      {withButton && (
        ...
        >
          <Skel w="100%" h={52} r={25} />
          {/* the centered reserveNote line under the CTA */}
          <Skel w="70%" h={13} r={7} />
        </div>
      )}
    </div>
            ...
            >
              ...
              >
                {Array.from({ length: 2 }).map((_, i) => (
                  ...
                  >
                    <Skel
                      w="var(--nk-size-icon-sm)"
                      h="var(--nk-size-icon-sm)"
                      r={12}
                    />
                    <div
                      style={{
              ...
              >
                <div
                  style={{
                    ...
                    gap: 10,
                    paddingRight: 44,
                    borderRight: "1px solid var(--nk-divider)",
                  }}
                >
                  <Skel w={72} h={52} r={10} />
                  <Skel w={96} h={16} />
...
export function ListingHeader({
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={metaItem}>
            ...
            {listing.rating ? (
              <>
                <b style={{ fontWeight: 700, color: "var(--nk-text)" }}>
                  {listing.rating}
                </b>
                <a
                  href="#atsiliepimai"
                ...
                >
                  ({dict.common.reviewCount(listing.ratingCount)})
                </a>
              </>
            ) : (
              <span style={{ color: "var(--nk-text-muted)" }}>
                {dict.detail.noReviewsYet}
              </span>
            )}
          </span>
          <Dot />
      ...
      <div
        className="nk-lhead__actions"
        style={{ display: "flex", gap: "var(--nk-gap-sm)" }}
      >
        <button className="nk-lfield" style={headerBtn} onClick={onShare}>
          <Icon name="Share2" size={17} stroke={2} color="var(--nk-text)" />{" "}
...
export function Gallery({
  ...
  const reviewPill = hasNoReviews ? (
    ...
    >
      <Icon name="Sparkles" size={14} color="var(--nk-yellow)" stroke={2} />{" "}
      {t.newListingPill}
    </span>
  ) : null;
          ...
          >
            {i === 0 && (
              <>
                {reviewPill}
                <span
                  className="nk-gtile__hint"
                  style={{
                    position: "absolute",
                    inset: 0,
                    borderRadius: "var(--nk-r-tile)",
                    background: "rgba(20,22,23,.18)",
                  }}
                />
            ...
            {i === shown.length - 1 && count >= 5 && (
              <span className="nk-gtile__more">
                <Icon
                  ...
                  stroke={2}
                />{" "}
                {extra > 0 ? t.galleryMore(extra) : t.galleryAll(count)}
              </span>
            )}
          </GalleryTile>
        ))}
...
function LightboxImage({
  ...
  return (
    <div className="nk-lightbox__img">
      {status !== "loaded" && (
        <span
          className="nk-imgph nk-lightbox__ph"
        ...
        >
          {status === "error" ? (
            ...
            <Icon
              ...
              size={30}
              stroke={2}
              className="nk-imgicon nk-lightbox__spin"
            />
          )}
        </span>
      )}
...
function DescriptionSection({ description }: { description: string }) {
  ...
  return (
    <Section id="aprasymas" title={t.descHeading} first>
      ...
      <div
        ref={bodyRef}
        className={"nk-prose" + (expanded ? "" : " nk-desc-clamp")}
        style={{
          margin: 0,
          maxWidth: "none",
          fontFamily: "var(--nk-font-body)",
  ...
  return (
    <div
      style={{
        ...
        justifyContent: "center",
        background:
          "radial-gradient(120% 90% at 50% 40%, var(--nk-bg) 0%, var(--nk-bg-deep) 100%)",
      }}
    >
      {/* faint road grid */}
      <span
      ...
      <span
        style={{
          ...
          border: "1.5px solid var(--nk-accent-border)",
        }}
      />
      <span
        style={{
      ...
      <span
        style={{
          ...
          alignItems: "center",
          justifyContent: "center",
          boxShadow: "0 8px 22px var(--nk-purple-glow-2)",
        }}
      >
...
function HandoverRow({
  ...
  return (
    <div
      style={{ display: "flex", alignItems: "center", gap: "var(--nk-gap-md)" }}
    >
      <span
        style={{
          width: "var(--nk-size-icon-sm)",
          height: "var(--nk-size-icon-sm)",
          borderRadius: 12,
          flex: "none",
          background: "var(--nk-purple-tag)",
          display: "flex",
      ...
      >
        <Icon name={icon} size={20} stroke={2} color="var(--nk-purple-hover)" />
      </span>
      <div
        style={{
          flex: 1,
...
function HandoverSection({
  ...
  return (
    <Section
      id="perdavimas"
      title={t.handoverHeading}
      sub={t.deliverySub(city, {
        pickup: showPickup,
        ...
        >
          {showPickup && (
            <HandoverRow
              ...
              pill={<Pill tone="green">{t.pickupFree}</Pill>}
            />
          )}
          {delivery.delivery && (
            <HandoverRow
              ...
              value={
                delivery.radiusKm
                  ? t.deliveryRadius(delivery.radiusKm)
                  : t.deliveryByArrangement
              }
              ...
              pill={
                delivery.pricePerKm ? (
                  <Pill tone="accent">
                    {t.deliveryPerKm(delivery.pricePerKm)}
                  </Pill>
                ) : undefined
              }
            />
...
function TermsSection({
  ...
  const applied =
    selectedDays !== null
      ? applicableDiscount(listing.discountTiers, selectedDays)
      : undefined;
  return (
    <Section id="salygos" title={t.termsHeading}>
      <div className="nk-hl-grid">
        ...
        <FactCard
          icon="ShieldCheck"
          title={
            listing.deposit ? t.depositTitle(listing.deposit) : t.depositNone
          }
          sub={t.termDepositSub}
        />
        <FactCard
          icon="Calendar"
          title={t.durationRange(listing.minDays, listing.maxDays)}
          sub={t.termDurationSub}
        />
        <FactCard
          icon="RefreshCcw"
          title={t.termCancelTitle(listing.cancellation)}
          sub={t.termCancelLabel}
          detail={t.termCancelDetail(listing.cancellation)}
        />
        {discounts.length > 0 && (
          <div className="nk-fact nk-breaks" style={{ gridColumn: "1 / -1" }}>
            <div className="nk-breaks__head">
              <span
                className="nk-fact__disk"
                style={{
                  width: 46,
              ...
              <span className="nk-breaks__headtext">
                <span className="nk-breaks__label">{t.discountsLabel}</span>
                <span className="nk-breaks__hint">
                  {selectedDays === null
                    ? t.discountsHintIdle
                    : applied
                      ? t.discountsHintActive({
                          days: selectedDays,
                          percent: applied.percent,
                        })
                      : t.discountsHintBelow({
                          days: selectedDays,
                          minDays: discounts[0].minDays,
                        })}
                </span>
              </span>
            </div>
            <ul className="nk-breaks__list">
              {discounts.map((tier) => {
                ...
                const active =
                  ...
                  applied.percent === tier.percent;
                return (
                  <li
                    key={tier.minDays}
                    className={
                      active ? "nk-breaks__tier is-active" : "nk-breaks__tier"
                    }
                  >
                    <span className="nk-breaks__top">
                      <span className="nk-breaks__from">
                        {t.discountFrom(tier.minDays)}
                      </span>
                      {active && (
                        <Icon
                          name="BadgeCheck"
                    ...
                    <span className="nk-breaks__prices">
                      <span className="nk-breaks__price nk-tnum">
                        {tier.perDay}
                      </span>
                      <span className="nk-breaks__unit">{t.perDayAbbr}</span>
                      <span className="nk-breaks__was nk-tnum">
                        {listing.price}{" "}
                        <span className="nk-breaks__wasunit">
                          {t.perDayAbbr}
                        </span>
                      </span>
                    </span>
...
function ReviewsBreakdown({
  ...
  return (
    ...
    >
                  ...
                  >
                    <span
                      style={{
                        ...
                        borderRadius: 4,
                        background:
                          "linear-gradient(90deg,var(--nk-yellow),var(--nk-yellow-deep))",
                      }}
                    />
                  </span>
                  <span
      ...
      <button
        className="nk-btn nk-btn--ghost"
        style={{ alignSelf: "flex-start", borderColor: "var(--nk-border)" }}
        onClick={onShowReviews}
      >
        {showAllLabel}{" "}
        <Icon name="ArrowRight" size={16} stroke={2} color="var(--nk-text)" />
      </button>
    </div>
  );
}
    ...
    <Section id="atsiliepimai" title={t.reviewsHeading}>
      {listing.ratingCount > 0 ? (
        <ReviewsBreakdown
          ...
          reviewCountLabel={dict.common.reviewCount(listing.ratingCount)}
          showAllLabel={t.reviewsInApp(listing.ratingCount)}
  ...
  return (
    ...
    >
      ...
      {listing.attributes.length > 0 && (
        <SpecsSection attributes={listing.attributes} />
      )}
      <HandoverSection
        city={listing.city}
      ...
      />
      <TermsSection listing={listing} selectedDays={selectedDays} />
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      {variant !== "facts" && (
        ...
        >
          {/* The dates ride into the app on this target — see listingAppPath().
              Reserve is never gated on picking them: the bridge's job is the
              handoff, and a picker that blocks the funnel would be a bug, not a
              feature. */}
          <a
            ...
            title={dict.bridge.opensAppHint}
            data-nk-redirect
            data-nk-redirect-title={dict.bridge.reserveTitle}
            data-nk-redirect-body={dict.bridge.reserveBody}
          ...
          <span className="nk-cta-note">{t.reserveNote}</span>
        </div>
      )}
    </div>
  );
...
export function HostCard({
  ...
  const ratingStat: HostStat = {
    value: rating ?? "—",
    label: t.hostStatRating,
    star: !!rating,
  };
  const reviewsStat: HostStat = {
    value: String(ratingCount),
    label: t.hostStatReviews,
  };
  const listingsStat: HostStat = {
    value: String(owner.listingsCount),
    label: t.hostStatListings,
  };
  ...
  return (
    ...
    >
        ...
        >
          ...
          {owner.verified && (
            <span style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              <Pill tone="green" icon="BadgeCheck" size="sm">
                {t.verifiedOwnerPill}
              </Pill>
            </span>
          )}
      ...
      {(owner.memberSince || owner.responseTimeHours != null) && (
        ...
        >
          {owner.memberSince && (
            <span>{t.hostMemberSince(owner.memberSince)}</span>
          )}
          {owner.responseTimeHours != null && (
            <span>{t.hostResponseTime(owner.responseTimeHours)}</span>
          )}
        </span>
      )}
      <div
      ...
      <button
        ...
        onClick={onContact}
        title={dict.bridge.opensAppHint}
        data-testid="nk-contact-owner"
        style={{ width: "100%", padding: "13px 24px", fontSize: 15.5 }}
      >
        <Icon
          name="MessageCircle"
          size={17}
...
export function MobileBar({
  ...
  return (
    <div className={"nk-mbar" + (hidden ? " is-hidden" : "")}>
      ...
      >
        <Icon name="Smartphone" size={16} stroke={2.2} color="var(--nk-text)" />{" "}
        {t.reserveMobile}
      </a>
    </div>
  );
...
export function feedCrumbItems({
  feedLabel,
  locale,
}: {
  feedLabel: string;
  ...
  return [
    ...
    ...(category && categoryId
      ? [
          {
            label: category,
            href: listingLandingHref({ category: categoryId, locale }),
          },
        ]
      : []),
    { label: title },
